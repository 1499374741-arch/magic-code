...
function resolveColor(value: string, container: HTMLElement) {
  ...
  return channels?.length === 3 ? { css: resolved, rgb: { r: channels[0], g: channels[1], b: channels[2] } } : null;
}

function resolveFontSize(value: number | string, container: HTMLElement, fontWeight: number, fontFamily: string) {
  if (typeof value === "number") return value;
  const probe = document.createElement("span");
  probe.textContent = "M";
  Object.assign(probe.style, { position: "absolute", visibility: "hidden", pointerEvents: "none", fontSize: value, fontWeight: String(fontWeight), fontFamily });
  container.appendChild(probe);
...
async function waitForFonts(font: string) {
  ...
  try { await document.fonts.load(font); } catch { /* Keep the system fallback when a font cannot load. */ }
...
export default function ParticleText({
  ...
  particleSize = 2,
  density = 4,
  color = "var(--particle-base-color)",
  highlightColor = "var(--particle-highlight-color)",
  scatter = 180,
  gatherDuration = 1600,
  stagger = 420,
  pointerRepel = 40,
  ...
  useEffect(() => {
    ...
    const resolvedGlow = resolveColor(highlightColor, container)?.css ?? highlightColor;
    ...
    const sampleText = async () => {
      ...
      particles = selected.map((target, index) => {
        ...
        const blend = baseColor && highlight ? clamp(target.x / Math.max(1, width) + (seed - 0.5) * 0.35, 0, 1) : 0;
        const particleColor = baseColor && highlight ? rgbToCss(mixRgb(baseColor.rgb, highlight.rgb, blend)) : baseColor?.css ?? color;
    ...
    const queueSample = () => {
      ...
      resizeFrame = window.requestAnimationFrame(() => { resizeFrame = null; void sampleText(); });
    };
    ...
    const handlePointerEnter = (event: PointerEvent) => { handlePointerMove(event); if (trigger === "hover") startGather(true); };
  ...
  return <div ref={containerRef} className={`particle-text ${className}`} style={style} role="img" aria-label={text}>
